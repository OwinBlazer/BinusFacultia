...
public class LevelLoader : MonoBehaviour {
    ...
    public void initSession()
    {
        ...
        if (sessionDetail.Length <= 0)
        {
            ...
            PlayerPrefs.SetString("sessionDetails","0#1#0");
        }
        else
        {
            //data found
            //set wave and smeester accordingly
    ...
    public void loadEnemies()
    {
        ...
        else
        {
            //spawn new units
            int maxRange = 0;
            foreach(SpawnGroup sg in semesterList[semester].spawnGroup)
            {
                maxRange += sg.value;
            }
            //roll rng
            ...
            int tempIndex = 0;
            while (rng > semesterList[semester].spawnGroup[tempIndex].value)
            {
                rng -= semesterList[semester].spawnGroup[tempIndex].value;
                tempIndex++;
            }
            foreach (EnemyChara eChara in semesterList[semester].spawnGroup[tempIndex].eCharaList)
            {
